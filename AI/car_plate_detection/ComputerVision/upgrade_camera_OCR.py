...
# YOLO 모델 로드
yolo_model = YOLO("Automatic-License-Plate-Recognition-using-YOLOv8-main/license_plate_detector.pt")
...
# Track the last three detected license plate texts
last_detected_texts = []
        ...
        for box in boxes:
            ...
            if int(box.cls) == 0:  # 번호판 클래스 ID를 확인하고 수정해야 합니다
                ...
                if ocr_result:
                    ...
                    # Update last detected texts list
                    last_detected_texts.append(plate_text)
                    if len(last_detected_texts) > 5:
                        last_detected_texts.pop(0)
                    
                    # Check if the last three detected texts are the same
                    if len(last_detected_texts) == 3 and all(text == last_detected_texts[0] for text in last_detected_texts):
                        capture_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        plate_texts.append((plate_text, capture_time))
                        last_detected_texts = []  # Reset after saving
...
# OCR 결과를 CSV 파일로 저장
with open('data/detected_license_plates.csv', mode='w', newline='') as file:
